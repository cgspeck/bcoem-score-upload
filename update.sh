...
echo "Getting name of folder in archive..."
# expect in form of 'foo-bar-baz/'
zip_subfolder=$(zipinfo -1 "${ZIP_DEST}" | head -n 1)
echo "Inner folder is '${zip_subfolder}'"
echo "Uncompressing archive..."
unzip "$ZIP_DEST" -d "$TMP"
echo "Copying files across..."
FROM="$TMP/$zip_subfolder/."
TO="."
cp -r "${FROM}" "${TO}"
