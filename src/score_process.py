...
def get_style_ids(
    cnn: MySQLConnection, entries: list[ScoreEntry], messages: list[str]
) -> None:
    """
...
def prepare_entries(
    ...
    if len(missing_entry_ids) > 0:
        ...
        format_error_message(
            f"Cannot find brewer ID for the following entries: {missing_entry_ids}",
            messages,
        )
    else:
    ...
    if len(missing_style_ids) > 0:
        ok = False
        format_error_message(
            f"Cannot find style IDs for the following entries: {missing_style_ids}",
            messages,
        )
    else:
    ...
    missing_judging_table_ids = [
        entry.entry_id for entry in entries if entry.style_id is None
    ]
    if len(missing_judging_table_ids) > 0:
        ok = False
        format_error_message(
            f"Cannot find judging table IDs for the following entries: {missing_judging_table_ids}",
            messages,
        )
    else:
    ...
    for r in dr:
        ...
        memo.append(
            ScoreEntry(
                ...
                appearance=float(r["Appearance"]),
                body=float(r["Body"]),
                aroma=float(r["Aroma"]),
                overall=float(r["Overall"]),
                flavour=float(r["Flavour"]),
                score_spread=float(r["Score Spread"]),
            )
        )
...
def save_entries(cnn: MySQLConnection, entries: list[ScoreEntry]) -> bool:
    ok = True
    sql = """
INSERT INTO judging_scores (eid, bid, scoreTable, scoreEntry, scorePlace, scoreType, wg_aroma, wg_appearance, wg_flavour, wg_body, wg_overall, wg_score_spread, wg_countback)
...
"""
    ...
    for entry in entries:
        data = (
            ...
            entry.overall,
            entry.score_spread,
            entry.countback_status_db_repr(),
        )
